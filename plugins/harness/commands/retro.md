...
1. **Decide if it's worth saving.** Keep only what would have saved real time had you known it
   up front. Skip anything the repository already records — code, git history, `CLAUDE.md`,
   `docs/architecture.md`, any path-scoped `CLAUDE.md` — and anything that mattered only to
   this one task.
...
4. **Escalate if it's recurring or procedural.** A one-off fact stays a memory. But if the
   ...
   - a durable standard or stack rule → an edit to `CLAUDE.md`, `docs/architecture.md`, or the
     path-scoped `CLAUDE.md` for the layer it governs;
