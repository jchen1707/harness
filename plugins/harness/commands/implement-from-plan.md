...
3. **Confirm the branch.** `/plan` creates the feature branch off the user's chosen base. Run
   ...
   use. Per `CLAUDE.md`, direct commits there need an explicit user request.
...
5. **Pin the layering.** Code lands where this repository's structure says it lands, not in a
   folder named after its kind. `docs/architecture.md` and the path-scoped `CLAUDE.md` for the
...
8. **Finish to the Definition of Done** in `CLAUDE.md`. Run `/verify` for the evidence — paste
...
> in `CLAUDE.md`. Prefer a text snapshot of the interface where one is available.
