...
2. **Check the rule files for staleness and duplication** — `CLAUDE.md`,
   `docs/architecture.md`, and any path-scoped `CLAUDE.md` this repository carries. A rule
...
3. **Check the tier.** Three exist and they are not interchangeable:
   ...
   - **`CLAUDE.md` and `docs/architecture.md`** — what has hardened into a rule everyone
     follows.
...
4. **Propose memory entries** — 1–3 concrete ones — for durable facts from this session:
   ...
   anything in `CLAUDE.md`. That is not memory, it is a stale second copy.
