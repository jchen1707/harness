...
file structure, its nested `CLAUDE.md` map and the vocabulary it has already settled.
...
- **`docs/architecture.md`** — **the decision record for the repo.** Its "choosing an
  ...
  would an ADR.
- **`CLAUDE.md`** — the always-loaded summary: approved stack, the repo's dependency or
  layering rule, Definition of Done. Where it and a skill's generic advice disagree,
  `CLAUDE.md` wins.
- **The nested `CLAUDE.md` for the directory you are changing**, where the repo has them.
...
**"agent" is the standing ambiguity in every one of these repos.** In `CLAUDE.md` and in these
