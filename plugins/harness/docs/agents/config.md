...
It is the layer rule from `CLAUDE.md` applied to executable content. Prose could split — the
...
- **`OBSIDIAN_VAULT_DIRECTORY`** — the **in-sandbox** variable, owned by the harness. It is
  ...
  read, and it is the only name the factory emits into a sandbox. A repo's own `CLAUDE.md`
