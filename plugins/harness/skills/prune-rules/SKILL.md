...
| Path-scoped `CLAUDE.md`        | Per-directory conventions, loaded only when working in that path.                                                  |
...
detail sitting in `architecture.md`. Path-scoped `CLAUDE.md` files are exactly that: an agent
working in one directory never loads another's. A rule that must hold everywhere belongs in
`architecture.md` or root `CLAUDE.md`, or it is not enforced where it matters.
